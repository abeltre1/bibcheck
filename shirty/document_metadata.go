...
func newFromTextRequest(text, model string) *openai.ChatRequest {
...
func newFromHtmlRequest(html, model string) *openai.ChatRequest {
...
func (c *Workflow) HTMLMetadata(html []byte) (*documents.Metadata, error) {
	return c.extractDocumentMetadataImpl(newFromHtmlRequest(string(html), c.model))
}

func (c *Workflow) TextMetadata(text string) (*documents.Metadata, error) {
	return c.extractDocumentMetadataImpl(newFromTextRequest(text, c.model))
}
